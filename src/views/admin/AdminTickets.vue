...
import { Trash, MoreHorizontal, Clock, CheckCircle, XCircle, ChevronDown, UserPlus } from "lucide-vue-next"
...
import { assignTicketToSupport } from "@/api/ticket"
...
import AssignUserModal from "@/components/AssignTicket.vue"
...
function getAssignStatus(ticket: TicketPreview) {
    if (!ticket.assigned_to) return "Unassigned";
    return "Assigned to " + ticket.assigned_user?.name;
}


function handleAssignTicket(ticket: TicketPreview) {
...
<template>
    <div class="space-y-6">
                                ...
                                <td class="py-2 px-3 text-right">
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent class="w-40 mr-5">
                                            <div class="px-3 py-1 text-[10px] text-center text-gray-500 border-b">
                                                {{ getAssignStatus(ticket) }}
                                            </div>

                                            <DropdownMenuItem @click="handleAssignTicket(ticket)"
                                                class="cursor-pointer">
                                                <UserPlus class="mr-2 h-4 w-4" />
                                                <span>
                                                    {{ ticket.assigned_to ? "Re-Assign Ticket" : "Assign Ticket" }}
                                                </span>
                                            </DropdownMenuItem>
        ...
        <Dialog v-model:open="isAssignDialogOpen">
            <DialogContent class="max-w-lg">
                <DialogHeader>
                    <DialogTitle>{{ selectedTicket?.assigned_to ? "Re-Assign Ticket" : "Assign Ticket" }}</DialogTitle>
                    <DialogDescription>
                        Choose a support user to assign this ticket.
                    </DialogDescription>
                </DialogHeader>
